...
<form name=frmEmpleados action="emplEditar_2.php" method=post>
<?
$iError = 0;
...
ORDER BY 1");
	...
	if ($accion == 'Editar Empleado'){
		print "Editando Empleado: $Apellido, $Nombre<br>";
	}else{
		print "Agregar Empleado<br><br>";
			
		$sqlLeg = "select ED.\"Legajo\" from \"tblEmpleadosDatos\" ED
                   INNER JOIN \"tblEmpleados\" E
                     ON ED.\"Legajo\" = E.\"Legajo\"
                     AND
                     ED.\"EmpresaID\" = E.\"EmpresaID\"
                     AND
                     ED.\"SucursalID\" = E.\"SucursalID\"

                   where (ED.\"FechaIngreso\" in (select MAX(\"FechaIngreso\") from \"tblEmpleadosDatos\" ED))
                     and
                     E.\"TipoRelacion\" not in (2)

                   UNION

                   select ED.\"Legajo\" from \"tblEmpleadosDatos\" ED 
                   INNER JOIN \"tblEmpleados\" E
                     ON ED.\"Legajo\" = E.\"Legajo\" 
                     AND
                     ED.\"EmpresaID\" = E.\"EmpresaID\"
                   ...
                   where (\"FechaIngreso\" in (select MAX(\"FechaIngreso\") from \"tblEmpleadosDatos\")) 
                     and
                     E.\"TipoRelacion\" in (2)";
...
$TipoEstudioID, '$estInstituto'::varchar, $estFechaFin, $estTitObt, $estCompletado)");
				}else{
...
// Se arma el codigo HTML del checkbox de estudio Completado y se presetea si esta tildado o no
		$EditarEst .= "<TR>
	<TD class='izquierdo'>Completado:</td><TD class=derecho><input type=\"checkbox\" id=chkCompletado$estTipoID name=chkCompletado$estTipoID value=1";
...
// Se arma el codigo HTML del checkbox de estudio Completado y se presetea si esta tildado o no
	$AgregarEst .= "<TR>
	<TD class='izquierdo'>Completado:</td><TD class=derecho><input type=\"checkbox\" id=chkCompletado$estTipoID name=chkCompletado$estTipoID value=1 unchecked>";
	$AgregarEst .= "</TD></TR>\n";
