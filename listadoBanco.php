...
<form name=frmListadoBanco action=listadoBanco.php method=post>
...
SELECT (case ed.\"LugarPago\" when 2 then '7134' when 3 then '7134' when 4 then '7135' when 5 then '7135' end),
    ...
    round(SUM(re.\"Haber1\"+re.\"Haber2\"-re.\"Descuento\")::numeric, 2) AS \"Neto\", p.\"TipoLiquidacionID\"
FROM \"tblRecibos\" re
...
ON em.\"EmpresaID\" = p.\"EmpresaID\" AND em.\"SucursalID\" = p.\"SucursalID\" AND re.\"Fecha\" = p.\"FechaPeriodo\" AND re.\"NumeroLiquidacion\" = p.\"NumeroLiquidacion\"
...
<?
	...
	while($row = pg_fetch_array($rs))
	{
		...
		$Nombre = $row[5];
		$SueldoDep = $row[6];
		$ApeYNom = trim(str_replace(array(',', '.'), ' ', $Apellido)) . ' ' . trim(str_replace(array(',', '.'), ' ', $Nombre));
		if ($SueldoDep == '')
			$SueldoDep = '0';
		$Importe = $SueldoDep;
