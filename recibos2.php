...
<form name=frmRecibos action=recibos2.php method=post style="display:inline">
<?
...
ON lp.\"EmpresaID\" = re.\"EmpresaID\" AND lp.\"LugarPago\" = ed.\"LugarPago\" AND lp.\"Activo\" = true
WHERE re.\"EmpresaID\" = $EmpresaID AND re.\"SucursalID\" = $SucursalID
AND re.\"Legajo\" = em.\"Legajo\" AND re.\"Fecha\" = '$FechaPeriodo'::date - interval '1 month' AND re.\"NumeroLiquidacion\" = $NumeroLiquidacion
ORDER BY ".($Jur=='1'?"er.jurisdiccion,":($LP == '1'?"ed.\"LugarPago\",":""))."1, \"Orden\", re.\"ConceptoID\", re.\"Descripcion\"");
	if (!$rs || !$rs2)
		exit;
	$Neto = 0;
	...
	while($row = pg_fetch_array($rs)){
		...
		if ((int)$row2[0] < (int)$row[0])
		{
			while ((int)$row2[0] < (int)$row[0])
			{
				$Concepto = $row2[2];
				$Descripcion = $row2[3];
				$Cantidad = $row2[4];
				...
				if ($Concepto == 99){
					$Neto2 = $Haber1 + $Haber2 - $Descuento;
				}
				?>
				<tr>
				<td colspan="6"></td><td bgcolor="#000000"></td>
		...
		if ($Legajo != $AntLegajo){
			if ($AntLegajo != ''){
				print "<TR><TD colspan=6 style='text-align:right'>NETO A COBRAR: <B>$Neto</B><br></TD><td bgcolor=\"#000000\"></td><TD colspan=6 style='text-align:right'>NETO A COBRAR: <B>$Neto2</B><br></TD></TR></table><br>\n";
			}
			$AntLegajo = $Legajo;
			if ($Jur == '1'){
...
?>
	...
	<tr><td colspan="12" style="text-align:left">Legajo Nro.: <B><?=$Legajo?></B>&nbsp;&nbsp;&nbsp;<B><?=$ApeYNom?></B>&nbsp;&nbsp;&nbsp;Fecha Ingreso: <B><?=$FechaIng?></B></td></tr>
		<th>Conc.</th><th>Descripcion</th><th>Cant.</th><th nowrap="nowrap">Haber c/desc</th><th nowrap="nowrap">Haber s/desc</th><th>Desc.</th><th></th>
