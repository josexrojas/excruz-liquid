...
//WHERE re.\"EmpresaID\" = $EmpresaID AND re.\"SucursalID\" = $SucursalID AND re.\"Legajo\" NOT IN ('3175') AND
	...
	$zipname = 'M036-1_'.$dMes.$dAno.'_ON_'.str_pad($NumeroLiquidacion, 2, '0', STR_PAD_LEFT).'.zip';
...
ON ca.\"EmpresaID\" = re.\"EmpresaID\" AND ca.\"AliasID\" = re.\"AliasID\" AND ca.\"AliasID\" NOT IN (29, 30, 31, 32, 261, 269)
...
ORDER BY \"ItemID\", \"NumeroLiquidacion\"
        ...
        if($RowIps == true && $ExisteIps == false){//  &&  $ExisteIps == false

          $ConcID_IPS = '80';
          $AliasID_IPS = str_pad('18', 8, ' ', STR_PAD_LEFT);
          $Descrip_IPS = str_pad('LEY 9650 I.P.S. 14%', 25, ' ', STR_PAD_LEFT);
          $CodIPS_IPS = '500000';
          $TipIPS_IPS = 'DPS';

          $NCargo_IPS = '0001';//$rowL['NumeroCargo'];//str_pad($rowL['NumeroCargo'], 2, "0", STR_PAD_LEFT);
    	  $NCargo_IPS = substr($NCargo_IPS, 0, 1).str_pad(substr($NCargo_IPS, 1), 2, "0", STR_PAD_LEFT);
          ...
          print str_pad($NCargo_IPS, 4, ' ', STR_PAD_RIGHT); // antes legajo num
          print str_pad($CodIPS_IPS, 6, ' ', STR_PAD_RIGHT);
          print str_pad($AliasID_IPS, 8, ' ', STR_PAD_RIGHT);
          print str_pad($TipIPS_IPS, 3, ' ', STR_PAD_RIGHT);
          print str_pad(substr(trim($Descrip_IPS), 0, 25), 25, ' ', STR_PAD_RIGHT);
          print str_pad((int)abs($IpsModificado), 7, '0', STR_PAD_LEFT).".".str_pad((int)abs(($IpsModificado*100) % 100), 2, '0', STR_PAD_LEFT);
 	  ...
 	  //$ExisteIps       = false;
				...
				if($ips == true && $LicenMaternidad > 0){
					$TotalLiq = $TotalLiq + ($rowL[4] + $rowL[5] - $rowL[6] - $IpsModificado);
					$Importe = ($rowL[4] + $rowL[5] - $rowL[6] - $IpsModificado);
				...
				if ($rowL[4] == ''){
					if ($rowL[5] == ''){
						if ($rowL[6] == ''){
							...
							if($ips == true && $AliasID == 18 && $LicenMaternidad > 0){
								$IpsModificado = $LicenMaternidad * 0.14;					
								$Importe   =  $rowL[6] + $LicenMaternidad * 0.14;
								$ExisteIps = true;
							}else{
								$Importe = $rowL[6];
							}
						...
						if($ips == true && $AliasID == 126)
							$LicenMaternidad = $rowL[5];

 							if($ips == true && $LicenMaternidad > 0 && $RowIps == false){ // Nuevo
                                                                $RowIps        = true;                     
								$IpsModificado = $LicenMaternidad * 0.14;
                                                         }
...
*/
	...
	$fp = fopen("/home/sitios/liquid/listados/salario.txt", "w");
	fwrite($fp, ob_get_contents());
	fclose($fp);

	ob_end_clean();

	exec("zip -j -r - /home/sitios/liquid/listados/salario.txt > /home/sitios/liquid/listados/".$zipname); 

	readfile("/home/sitios/liquid/listados/".$zipname);
	exit;
